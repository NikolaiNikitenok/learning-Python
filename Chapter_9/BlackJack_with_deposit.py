...
class BJ_Player(BJ_Hand):
    ...
    def win(self):
        ...
        self.plus(self.winning)
        print(self.winning)

    def push(self):
        print(self.name, "сыграл с компьютером в ничью.")
        self.plus(self.betik)
        
    def bet(self, player):
        self.betik = int(input(f"\n{player.name}, какую ставку вы сделаете? (1 - 100): "))
        player.minus(self.betik)
        self.winning = self.betik * 2
        print(player)
    ...
    def play(self): 
        ...
        for player in self.players:
            if player.money > 0:
                ...
                print(f"{player.name}, на вашем счете недостаточно средств!")
        print()
